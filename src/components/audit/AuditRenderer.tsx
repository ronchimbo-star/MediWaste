    ...
    <div id="audit-render" className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="px-8 py-6 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          {companyNumber && (
            <div><span className="text-gray-500 font-medium">Company No:</span> <span className="text-gray-800 font-mono">{companyNumber}</span></div>
          )}
          {auditNumber && (
          ...
          <Section number="3" title="Detailed Waste Stream Assessment" icon={<Award className="w-4 h-4" />}>
            {content.detailed_assessment.map((stream, idx) => (
              <div key={idx} className="mb-6">
                <h4 className="text-sm font-semibold text-[#dc2626] mb-2">3.{idx + 1} {stream.name}</h4>
                ...
                <td key={ci} className="px-3 py-2 border border-gray-200 text-gray-700">
                  {editable && onCellChange ? (
                    <input
                      ...
                      className="w-full bg-transparent focus:bg-red-50 focus:outline-none px-1 py-0.5 rounded"
                    />
                  ) : (
